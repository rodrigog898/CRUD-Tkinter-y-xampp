...
class App12345:
		...
		def DrawLabel(self):
			...
			self.lbl_fecha = Label(self.frame, foreground="white",font=(8), background="#314252", text="Fecha Retiro").place(x=200, y=260)
			self.lbl_desc = Label(self.frame, foreground="white",font=(8), background="#314252", text="Descripcion").place(x=200, y=310)
		...
		def DrawEntry(self):
			...
			self.txt_descripcion= Entry(self.frame,font=('Arial', 12),relief="flat", background="#E7E7E7" ,textvariable=self.descripcion).place(x=310, y=310, height=25, width=150)
		...
		def DrawList(self):
			...
			self.list_elemts.heading(3, text="Rut"),self.list_elemts.heading(4, text="Fecha")
			self.list_elemts.heading(5, text="Descripcion")
		...
		def confirmProcess(self):
			if self.id.get() != "" and self.nombre.get() != "" and self.rut.get() != "" and self.fecha.get() != ""and self.descripcion.get() != "":
				d = Data()
				arr = [self.id.get(), self.nombre.get(), self.rut.get(), self.fecha.get(), self.descripcion.get()]
				d.InsertItems(arr)
